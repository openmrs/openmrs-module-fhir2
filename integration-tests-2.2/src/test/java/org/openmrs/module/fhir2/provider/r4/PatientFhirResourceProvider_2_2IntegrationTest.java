...
public class PatientFhirResourceProvider_2_2IntegrationTest extends BaseFhirR4IntegrationTest<PatientFhirResourceProvider, Patient> {
	...
	public void shouldPatchExistingPatientViaJson() throws Exception {
		...
		MockHttpServletResponse response = patch("/Patient/" + PATIENT_UUID)
				.jsonMergePatch(jsonPatientPatch).accept(FhirMediaTypes.JSON).go();
		...
		assertThat(patient.getBirthDate(),is("2000-01-26"));
//		assertThat(patient.getName(),is("2000-01-26"));
	}
}
