...
public class MedicationDispenseFhirResourceProviderTest {
	...
	public void setup() {
		...
		when(fhirMedicationDispenseService.searchMedicationDispenses(any())).thenReturn(
				new MockIBundleProvider<>(Collections.singletonList(medicationDispense), 10, 1)
		);
	...
	protected void testSearch() {
		IBundleProvider results = resourceProvider.searchForMedicationDispenses(
				idParam, patientParam, subjectParam, encounterParam, medicationRequestParam, lastUpdatedParam, includeParam, sortParam
		);
