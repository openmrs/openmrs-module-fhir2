...
public class ConditionFhirResourceProviderTest extends BaseFhirProvenanceResourceTest<Condition> {
	...
	//		when(conditionService.delete(WRONG_CONDITION_UUID)).thenReturn(condition);
	//		OperationOutcome result = resourceProvider.deleteCondition(new IdType().setValue(WRONG_CONDITION_UUID));
	//		assertThat(result, notNullValue());
