...
public class ConditionFhirResourceProviderTest extends BaseFhirProvenanceResourceTest<Condition> {
	...
	public void updateCondition_shouldThrowInvalidRequestForUuidMismatch() {
		when(conditionService.update(WRONG_CONDITION_UUID, condition)).thenThrow(InvalidRequestException.class);
		
		resourceProvider.updateCondition(new IdType().setValue(WRONG_CONDITION_UUID), condition);
	}
	...
	@Test(expected = ResourceNotFoundException.class)
	public void deleteCondition_shouldThrowResourceNotFoundExceptionWhenIdRefersToNonExistentCondition() {
