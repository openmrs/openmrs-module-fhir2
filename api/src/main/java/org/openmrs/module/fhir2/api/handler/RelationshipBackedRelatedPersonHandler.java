...
/**
 * Default handler for the FHIR {@link RelatedPerson} resource, mapping onto the OpenMRS
 * {@code Relationship} table. Single backing today — claims every incoming RelatedPerson. Carries
 * the CRUD wiring lifted from the original {@code FhirRelatedPersonServiceImpl}.
 */
@Component
@Order(Ordered.LOWEST_PRECEDENCE)
public class RelationshipBackedRelatedPersonHandler extends BaseFhirResourceHandler<RelatedPerson, Relationship> implements FhirResourceHandler<RelatedPerson> {
	
	private static final String IMPLICIT_PROFILE = OPENMRS_FHIR_STRUCTURE_DEFINITION_PREFIX + "/openmrs-relatedperson";
	...
	@Override
	public boolean canHandle(@Nonnull RelatedPerson resource) {
		return true;
	}
