...
/**
 * Default handler for the FHIR {@link Task} resource, mapping onto the OpenMRS {@link FhirTask}
 * table. Single backing today — claims every incoming Task. Carries the CRUD wiring lifted from the
 * original {@code FhirTaskServiceImpl}.
 */
@Component
@Order(Ordered.LOWEST_PRECEDENCE)
public class FhirTaskBackedTaskHandler extends BaseFhirResourceHandler<Task, FhirTask> implements FhirResourceHandler<Task> {
	
	private static final String IMPLICIT_PROFILE = OPENMRS_FHIR_STRUCTURE_DEFINITION_PREFIX + "/openmrs-task";
	...
	@Override
	public boolean canHandle(@Nonnull Task resource) {
		return true;
	}
