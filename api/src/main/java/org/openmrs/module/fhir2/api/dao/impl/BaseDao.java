...
public abstract class BaseDao {
     ...
     * parameters
     */
    protected <T extends IQueryParameterOr<U>, U extends IQueryParameterType> Optional<Predicate> handleAndListParam(CriteriaBuilder criteriaBuilder, IQueryParameterAnd<T> andListParam, Function<U, Optional<Predicate>> handler) {
        if (andListParam == null) {
            return Optional.empty();
        }

        return Optional.ofNullable(criteriaBuilder.and(toCriteriaArray(handleAndListParam(andListParam).map(orListParam -> handleOrListParam(criteriaBuilder,orListParam, handler)))));
    }

    @SuppressWarnings("unused")
    protected <T extends IQueryParameterOr<U>, U extends IQueryParameterType> Optional<Predicate> handleAndListParamBy(CriteriaBuilder criteriaBuilder, IQueryParameterAnd<T> andListParam, Function<IQueryParameterOr<U>, Optional<Predicate>> handler) {
        if (andListParam == null) {
            return Optional.empty();
        }
    ...
    protected <T extends IQueryParameterOr<U>, U extends IQueryParameterType> Optional<Predicate> handleAndListParamAsStream(CriteriaBuilder criteriaBuilder, IQueryParameterAnd<T> andListParam, Function<U, Stream<Optional<Predicate>>> handler) {
        if (andListParam == null) {
            return Optional.empty();
        }
     ...
     */
    protected <T extends IQueryParameterType> Optional<Predicate> handleOrListParam(CriteriaBuilder criteriaBuilder, IQueryParameterOr<T> orListParam, Function<T, Optional<Predicate>> handler) {
        if (orListParam == null) {
            return Optional.empty();
        }
    ...
    protected <T extends IQueryParameterType> Optional<Predicate> handleOrListParamAsStream(CriteriaBuilder criteriaBuilder, IQueryParameterOr<T> orListParam, Function<T, Stream<Optional<Predicate>>> handler) {
        if (orListParam == null) {
            return Optional.empty();
        }
     ...
     */
    protected <T extends IQueryParameterOr<TokenParam>> Optional<Predicate> handleAndListParamBySystem(CriteriaBuilder criteriaBuilder, IQueryParameterAnd<T> andListParam, BiFunction<String, List<TokenParam>, Optional<Predicate>> systemTokenHandler) {
        if (andListParam == null) {
            return Optional.empty();
        }
     ...
     */
    protected Optional<Predicate> handleOrListParamBySystem(CriteriaBuilder criteriaBuilder, IQueryParameterOr<TokenParam> orListParam, BiFunction<String, List<TokenParam>, Optional<Predicate>> systemTokenHandler) {
     ...
     * given value
     */
    protected <T> Optional<Predicate> handleBoolean(OpenmrsFhirCriteriaContext<T> criteriaContext, String propertyName, TokenAndListParam booleanToken) {
    ...
    protected <T> Optional<Predicate> handleBooleanProperty(OpenmrsFhirCriteriaContext<T> criteriaContext, String propertyName, boolean booleanVal) {
     ...
     */
    protected <T> Optional<Predicate> handleDateRange(OpenmrsFhirCriteriaContext<T> criteriaContext, String propertyName, DateRangeParam dateRangeParam) {
     ...
     */
    protected <T> Optional<Predicate> handleDate(OpenmrsFhirCriteriaContext<T> criteriaContext, String propertyName, DateParam dateParam) {
        if (dateParam == null) {
            return Optional.empty();
        }
    ...
    protected <T> Optional<Predicate> handleQuantity(OpenmrsFhirCriteriaContext<T> criteriaContext, String propertyName, QuantityParam quantityParam) {
    ...
    protected <T> Optional<Predicate> handleQuantity(OpenmrsFhirCriteriaContext<T> criteriaContext, @Nonnull String propertyName, QuantityAndListParam quantityAndListParam) {
    ...
    protected <T> void handleEncounterReference(OpenmrsFhirCriteriaContext<T> criteriaContext, ReferenceAndListParam encounterReference, @Nonnull String encounterAlias) {
        handleEncounterReference(criteriaContext, encounterReference, encounterAlias, "encounter");
    }

    protected <T> void handleEncounterReference(OpenmrsFhirCriteriaContext<T> criteriaContext, ReferenceAndListParam encounterReference, @Nonnull String encounterAlias, @Nonnull String associationPath) {
            ...
            if (token.getChain() != null) {
                switch (token.getChain()) {
                    case Encounter.SP_TYPE:
                        Join<?, ?> associationPathEncounterEncounterTypeJoin = criteriaContext.addJoin(associationPathEncounterJoin,"encounterType", "et");
                        return propertyLike(criteriaContext, associationPathEncounterEncounterTypeJoin, "uuid", new StringParam(token.getValue(), true));
                }
            } else {
                return Optional.of(criteriaContext.getCriteriaBuilder().equal(associationPathEncounterJoin.get("uuid"), token.getIdPart()));
            }
    ...
    protected <T> Optional<Predicate> handleGender(OpenmrsFhirCriteriaContext<T> criteriaContext, @Nonnull String propertyName, TokenAndListParam gender) {
        if (gender == null) {
            return Optional.empty();
        }
        ...
        return handleAndListParam(criteriaContext.getCriteriaBuilder(), gender, token -> {
            try {
                ...
                if (administrativeGender == null) {
                    return Optional.of(criteriaContext.getCriteriaBuilder().isNull(criteriaContext.getRoot().get(propertyName)));
                }

                switch (administrativeGender) {
                    case MALE:
                        return Optional.of(criteriaContext.getCriteriaBuilder().like(criteriaContext.getRoot().get(propertyName), "M"));
                    case FEMALE:
                        return Optional.of(criteriaContext.getCriteriaBuilder().like(criteriaContext.getRoot().get(propertyName), "F"));
                    case OTHER:
                    case UNKNOWN:
                    case NULL:
                        return Optional.of(criteriaContext.getCriteriaBuilder().isNull(criteriaContext.getRoot().get(propertyName)));
            ...
            return Optional.of(criteriaContext.getCriteriaBuilder().like(criteriaContext.getRoot().get(propertyName), token.getValue()));
        });
    }

    protected <T> Optional<Predicate> handleLocationReference(OpenmrsFhirCriteriaContext<T> criteriaContext, @Nonnull String locationAlias, ReferenceAndListParam locationReference) {
            ...
            if (token.getChain() != null) {
                switch (token.getChain()) {
                    case Location.SP_NAME:
                        return propertyLike(criteriaContext, getRootOrJoin(criteriaContext, locationAlias), "name", token.getValue());
                    case Location.SP_ADDRESS_CITY:
                        return propertyLike(criteriaContext, getRootOrJoin(criteriaContext, locationAlias), "cityVillage", token.getValue());
                    case Location.SP_ADDRESS_STATE:
                        return propertyLike(criteriaContext, getRootOrJoin(criteriaContext, locationAlias), "stateProvince", token.getValue());
                    case Location.SP_ADDRESS_POSTALCODE:
                        return propertyLike(criteriaContext, getRootOrJoin(criteriaContext, locationAlias), "postalCode", token.getValue());
                    case Location.SP_ADDRESS_COUNTRY:
                        return propertyLike(criteriaContext, getRootOrJoin(criteriaContext, locationAlias), "country", token.getValue());
                }
            } else {
                return Optional.of(criteriaContext.getCriteriaBuilder().equal(criteriaContext.getRoot().get(String.format("%s..uuid", locationAlias)), token.getValue()));
            }
    ...
    protected <T> void handleParticipantReference(OpenmrsFhirCriteriaContext<T> criteriaContext, ReferenceAndListParam participantReference) {
                ...
                if (participantToken.getChain() != null) {
                    switch (participantToken.getChain()) {
                        case Practitioner.SP_IDENTIFIER:
                            ...
                            return criteriaContext.getJoin("p").map(providerJoin -> criteriaContext.getCriteriaBuilder().like(providerJoin.get("identifier"), participantToken.getValue()));
                        case Practitioner.SP_GIVEN: {
                            Join<?, ?> encounterProviderProvider = criteriaContext.addJoin(encounterProvider.get(), "provider", "pro");
                        ...
                        case Practitioner.SP_FAMILY: {
                            ...
                            return Optional.of(criteriaContext.getCriteriaBuilder().like(encounterProviderPersonName.get("familyName"), participantToken.getValue()));
                        }
                        case Practitioner.SP_NAME: {
                            ...
                            for (String token : StringUtils.split(participantToken.getValue(), " \t,")) {
                                predicateList.add(propertyLike(criteriaContext, encounterProviderPersonName, "givenName", token));
                                predicateList.add(propertyLike(criteriaContext, encounterProviderPersonName, "middleName", token));
                                predicateList.add(propertyLike(criteriaContext, encounterProviderPersonName, "familyName", token));
                            }
                        ...
                        Join<?, ?> encounterProviderProvider = criteriaContext.addJoin(encounterProvider.get(), "provider", "pro");
                    return Optional.of(criteriaContext.getCriteriaBuilder().equal(encounterProviderProvider.get("pro.uuid"), participantToken.getValue()));
                }
    ...
    protected <T> void handleProviderReference(OpenmrsFhirCriteriaContext<T> criteriaContext, ReferenceAndListParam providerReference) {
        if (providerReference != null) {
            ...
            handleAndListParam(criteriaContext.getCriteriaBuilder(), providerReference, participantToken -> {
                if (participantToken.getChain() != null) {
                    switch (participantToken.getChain()) {
                        case Practitioner.SP_IDENTIFIER:
                            return Optional.of(criteriaContext.getCriteriaBuilder().like(orderer.get("identifier"), participantToken.getValue()));
                        case Practitioner.SP_GIVEN: {
                            ...
                            return Optional.of(criteriaContext.getCriteriaBuilder().like(ordererName.get("givenName"), participantToken.getValue()));
                        }
                        case Practitioner.SP_FAMILY: {
                            Join<?, ?> ordererPerson = criteriaContext.addJoin(orderer, "person", "ps");
                            Join<?, ?> ordererName = criteriaContext.addJoin(ordererPerson, "names", "pn");

                            return Optional.of(criteriaContext.getCriteriaBuilder().like(ordererName.get("pn.familyName"), participantToken.getValue()));
                        }
                        case Practitioner.SP_NAME: {
                            Join<?, ?> ordererPerson = criteriaContext.addJoin(orderer, "person", "ps");
                            Join<?, ?> ordererName = criteriaContext.addJoin(ordererPerson, "names", "pn");
    ...
    protected <T> Optional<Predicate> handleCodeableConcept(OpenmrsFhirCriteriaContext<T> criteriaContext, TokenAndListParam concepts, @Nonnull String conceptAlias, @Nonnull String conceptMapAlias, @Nonnull String conceptReferenceTermAlias) {
        if (concepts == null) {
            return Optional.empty();
        }
        Optional<Join<?, ?>> conceptAliasJoin = criteriaContext.getJoin(conceptAlias);
	    return conceptAliasJoin.map(
			    join -> handleAndListParamBySystem(criteriaContext.getCriteriaBuilder(), concepts, (system, tokens) -> {
				    if (system.isEmpty()) {
					    criteriaContext.getCriteriaBuilder()
							    .literal(tokensToParams(tokens).map(NumberUtils::toInt).collect(Collectors.toList()));
					    return Optional.of(criteriaContext.getCriteriaBuilder().or(criteriaContext.getCriteriaBuilder()
									    .in(join.get("conceptId").in(criteriaContext.getCriteriaBuilder().literal(
					    ...
					    return Optional.of(generateSystemQuery(criteriaContext, system, tokensToList(tokens),
							    conceptReferenceTermAlias));
				    }
			    })).orElse(null);
	    
    }

    protected <T> void handleNames(OpenmrsFhirCriteriaContext<T> criteriaContext, StringAndListParam name, StringAndListParam given, StringAndListParam family) {
        handleNames(criteriaContext, name, given, family, null);
    }

    protected <T> void handleNames(OpenmrsFhirCriteriaContext<T> criteriaContext, StringAndListParam name, StringAndListParam given, StringAndListParam family, String personAlias) {
        ...
        if (name != null) {
            handleAndListParamAsStream(criteriaContext.getCriteriaBuilder(), name, (nameParam) -> Arrays.stream(StringUtils.split(nameParam.getValue(), " \t,")).map(token -> new StringParam().setValue(token).setExact(nameParam.isExact()).setContains(nameParam.isContains())).map(tokenParam -> Arrays.asList(
                    ...
                    propertyLike(criteriaContext, personNameAliasJoin,"familyName", tokenParam))).flatMap(Collection::stream)).ifPresent(criteriaContext::addPredicate);
            criteriaContext.finalizeQuery();
        }

        if (given != null) {
            handleAndListParam(criteriaContext.getCriteriaBuilder(),given, (givenName) -> propertyLike(criteriaContext, personNameAliasJoin,"givenName", givenName)).ifPresent(criteriaContext::addPredicate);
            criteriaContext.finalizeQuery();
        }

        if (family != null) {
            handleAndListParam(criteriaContext.getCriteriaBuilder(),family, (familyName) -> propertyLike(criteriaContext, personNameAliasJoin,"familyName", familyName)).ifPresent(criteriaContext::addPredicate);
            criteriaContext.finalizeQuery();
        }
    }

    protected <T> void handlePatientReference(OpenmrsFhirCriteriaContext<T> criteriaContext, ReferenceAndListParam patientReference) {
        handlePatientReference(criteriaContext, patientReference, "patient");
    }

    protected <T> void handlePatientReference(OpenmrsFhirCriteriaContext<T> criteriaContext, ReferenceAndListParam patientReference, String associationPath) {
                ...
                if (patientToken.getChain() != null) {
                    switch (patientToken.getChain()) {
                        case Patient.SP_IDENTIFIER:
                            Join<?, ?> associationPathIdentifiersJoin =  criteriaContext.addJoin(associationPathJoin,"identifiers", "pi");
                            return Optional.of(criteriaContext.getCriteriaBuilder().like(associationPathIdentifiersJoin.get("identifier"), patientToken.getValue()));
                        ...
                        case Patient.SP_NAME:
                            Join<?, ?> associationPathNamesJoin =  criteriaContext.addJoin(associationPathJoin,"names", "pn");

                            List<Optional<? extends Predicate>> criterionList = new ArrayList<>();

                            for (String token : StringUtils.split(patientToken.getValue(), " \t,")) {
                                criterionList.add(propertyLike(criteriaContext, associationPathNamesJoin,"givenName", token));
                                criterionList.add(propertyLike(criteriaContext, associationPathNamesJoin,"middleName", token));
                                criterionList.add(propertyLike(criteriaContext, associationPathNamesJoin,"familyName", token));
                            }
                            return Optional.of(criteriaContext.getCriteriaBuilder().or(toCriteriaArray(criterionList)));
                    }
                } else {
                    return Optional.of(criteriaContext.getCriteriaBuilder().equal(associationPathJoin.get("uuid"), patientToken.getValue()));
                }
    ...
    protected <T> Optional<Predicate> handleCommonSearchParameters(OpenmrsFhirCriteriaContext<T> criteriaContext, List<PropParam<?>> theCommonParams) {
        List<Optional<? extends Predicate>> criterionList = new ArrayList<>();

        for (PropParam<?> commonSearchParam : theCommonParams) {
            switch (commonSearchParam.getPropertyName()) {
                case FhirConstants.ID_PROPERTY:
                    criterionList.add(handleAndListParam(criteriaContext.getCriteriaBuilder(), (TokenAndListParam) commonSearchParam.getParam(), param -> Optional.of(criteriaContext.getCriteriaBuilder().equal(criteriaContext.getRoot().get("uuid"), param.getValue()))));
                    break;
     ...
     */
    protected abstract <T> Optional<Predicate> handleLastUpdated(OpenmrsFhirCriteriaContext<T> criteriaContext, DateRangeParam param);

    protected <T> Optional<Predicate> handlePersonAddress(OpenmrsFhirCriteriaContext<T> criteriaContext, String aliasPrefix, StringAndListParam city, StringAndListParam state, StringAndListParam postalCode, StringAndListParam country) {
        ...
        if (city != null) {
            predicateList.add(handleAndListParam(criteriaContext.getCriteriaBuilder(), city, c -> propertyLike(criteriaContext, getRootOrJoin(criteriaContext,aliasPrefix),"cityVillage", c)));
        }

        if (state != null) {
            predicateList.add(handleAndListParam(criteriaContext.getCriteriaBuilder(), state, c -> propertyLike(criteriaContext, getRootOrJoin(criteriaContext,aliasPrefix),"stateProvince", c)));
        }

        if (postalCode != null) {
            predicateList.add(handleAndListParam(criteriaContext.getCriteriaBuilder(), postalCode, c -> propertyLike(criteriaContext, getRootOrJoin(criteriaContext,aliasPrefix),"postalCode", c)));
        }

        if (country != null) {
            predicateList.add(handleAndListParam(criteriaContext.getCriteriaBuilder(), country, c -> propertyLike(criteriaContext, getRootOrJoin(criteriaContext,aliasPrefix),"country", c)));
        }

        return Optional.of(criteriaContext.getCriteriaBuilder().and(toCriteriaArray(predicateList.stream())));
    }

    protected <T> Optional<Predicate> handleMedicationReference(OpenmrsFhirCriteriaContext<T> criteriaContext, @Nonnull String medicationAlias, ReferenceAndListParam medicationReference) {
        if (medicationReference == null) {
            return Optional.empty();
        }
        
        return handleAndListParam(criteriaContext.getCriteriaBuilder(), medicationReference, token -> Optional.of(criteriaContext.getCriteriaBuilder().equal(getRootOrJoin(criteriaContext,medicationAlias).get("uuid"), token.getIdPart())));
    }

    protected <T> Optional<Predicate> handleMedicationRequestReference(OpenmrsFhirCriteriaContext<T> criteriaContext, @Nonnull String drugOrderAlias, ReferenceAndListParam drugOrderReference) {
        if (drugOrderReference == null) {
            return Optional.empty();
        }

        return handleAndListParam(criteriaContext.getCriteriaBuilder(), drugOrderReference, token -> Optional.of(criteriaContext.getCriteriaBuilder().equal(getRootOrJoin(criteriaContext,drugOrderAlias).get("uuid"), token.getIdPart())));
    }
    ...
    protected <T> Optional<List<javax.persistence.criteria.Order>> handleSort(OpenmrsFhirCriteriaContext<T> criteriaContext, SortSpec sort, BiFunction<OpenmrsFhirCriteriaContext<T>, SortState, Collection<javax.persistence.criteria.Order>> paramToProp) {
        List<javax.persistence.criteria.Order> orderings = new ArrayList<>();
        SortSpec sortSpec = sort;
        while (sortSpec != null) {
            SortState state = SortState.builder().criteriaBuilder(criteriaContext.getCriteriaBuilder()).sortOrder(sortSpec.getOrder()).parameter(sortSpec.getParamName().toLowerCase()).build();
    ...
    protected <T> Predicate generateSystemQuery(OpenmrsFhirCriteriaContext<T> criteriaContext, String system, List<String> codes, String conceptReferenceTermAlias) {
        //detached criteria
        Specification<FhirConceptSource> spec = (root, query, cb) -> (Predicate) query.select(root.get("conceptSource")).where(cb.equal(root.get("url"), system));

        criteriaContext.getCriteriaQuery().where(spec.toPredicate((Root<FhirConceptSource>) criteriaContext.getRoot(), (CriteriaQuery<FhirConceptSource>) criteriaContext.getCriteriaQuery(), criteriaContext.getCriteriaBuilder()));

        if (codes.size() > 1) {
            return criteriaContext.getCriteriaBuilder().and(criteriaContext.getCriteriaBuilder().equal(getRootOrJoin(criteriaContext,conceptReferenceTermAlias).get("conceptSource"), criteriaContext.getCriteriaQuery()), criteriaContext.getCriteriaBuilder().in(criteriaContext.getRoot().get(String.format("%s.code", conceptReferenceTermAlias)).get(codes.toString())));
        } else {
    ...
    protected <T> Predicate generateActiveOrderQuery(OpenmrsFhirCriteriaContext<T> criteriaContext, String path, Date onDate) {
        ...
        return criteriaContext.getCriteriaBuilder().and(criteriaContext.getCriteriaBuilder().or(criteriaContext.getCriteriaBuilder()
                .isNull(getRootOrJoin(criteriaContext,path).get("dateActivated")), criteriaContext.getCriteriaBuilder()
                .lessThan(getRootOrJoin(criteriaContext,path).get("dateActivated"), onDate)), criteriaContext.getCriteriaBuilder()
                .or(criteriaContext.getCriteriaBuilder().isNull(getRootOrJoin(criteriaContext,path).get("dateStopped")), criteriaContext.getCriteriaBuilder()
                        .greaterThan(getRootOrJoin(criteriaContext,path).get("dateStopped"), onDate)), criteriaContext.getCriteriaBuilder()
                .or(criteriaContext.getCriteriaBuilder().isNull(getRootOrJoin(criteriaContext,path).get("autoExpireDate")), criteriaContext.getCriteriaBuilder()
                        .greaterThan(getRootOrJoin(criteriaContext,path).get("autoExpireDate"), onDate)));
    }
    ...
    protected <T> Predicate generateNotCancelledOrderQuery(OpenmrsFhirCriteriaContext<T> criteriaContext, String path) {
        ...
        return criteriaContext.getCriteriaBuilder().or(criteriaContext.getCriteriaBuilder().isNull(getRootOrJoin(criteriaContext,path).get("dateStopped")), criteriaContext.getCriteriaBuilder().greaterThan(getRootOrJoin(criteriaContext,path).get("dateStopped"), now));
    }
    ...
    protected TokenOrListParam convertStringStatusToBoolean(TokenOrListParam statusParam) {
        if (statusParam != null) {
            ...
            }).filter(Optional::isPresent).map(Optional::get).collect(TokenOrListParam::new, (tp, v) -> tp.add(String.valueOf(v)), (tp1, tp2) -> tp2.getListAsCodings().forEach(tp1::add));
        }
    ...
    protected TokenAndListParam convertStringStatusToBoolean(TokenAndListParam statusParam) {
        if (statusParam != null) {
            return handleAndListParam(statusParam).map(this::convertStringStatusToBoolean).collect(TokenAndListParam::new, TokenAndListParam::addAnd, (tp1, tp2) -> tp2.getValuesAsQueryTokens().forEach(tp1::addAnd));
        }
    ...
    protected <T> Collection<Order> paramToProps(OpenmrsFhirCriteriaContext<T> criteriaContext, @Nonnull SortState sortState) {
        ...
        if (prop != null) {
            switch (sortState.getSortOrder()) {
                case ASC:
                    return prop.stream().map(s -> criteriaContext.getCriteriaBuilder().asc(criteriaContext.getRoot().get(s))).collect(Collectors.toList());
                case DESC:
                    return prop.stream().map(s -> criteriaContext.getCriteriaBuilder().desc(criteriaContext.getRoot().get(s))).collect(Collectors.toList());
            }
        }
    ...
    protected <T> Optional<Predicate> propertyLike(OpenmrsFhirCriteriaContext<T> criteriaContext, From<?, ?> from, @Nonnull String propertyName, String value) {
        if (value == null) {
            return Optional.empty();
        }
    ...
    protected <T> Optional<Predicate> propertyLike(OpenmrsFhirCriteriaContext<T> criteriaContext, From<?, ?> from, @Nonnull String propertyName, StringParam param) {
        if (param == null) {
            return Optional.empty();
        }
    ...
    protected <T extends IQueryParameterOr<U>, U extends IQueryParameterType> Stream<T> handleAndListParam(IQueryParameterAnd<T> andListParameter) {
    ...
    protected <T> Optional<Predicate> handleAgeByDateProperty(OpenmrsFhirCriteriaContext<T> criteriaContext, @Nonnull String datePropertyName, @Nonnull QuantityParam age) {
        ...
        if (prefix == ParamPrefixEnum.EQUAL || prefix == ParamPrefixEnum.NOT_EQUAL) {
            ...
            if (prefix == ParamPrefixEnum.EQUAL) {
                return Optional.ofNullable(criteriaContext.getCriteriaBuilder().and(criteriaContext.getCriteriaBuilder().greaterThanOrEqualTo(criteriaContext.getRoot().get(datePropertyName), lowerBound), criteriaContext.getCriteriaBuilder().lessThanOrEqualTo(criteriaContext.getRoot().get(datePropertyName), upperBound)));
            } else {
                return Optional.ofNullable(criteriaContext.getCriteriaBuilder().not(criteriaContext.getCriteriaBuilder().and(criteriaContext.getCriteriaBuilder().greaterThanOrEqualTo(criteriaContext.getRoot().get(datePropertyName), lowerBound), criteriaContext.getCriteriaBuilder().lessThanOrEqualTo(criteriaContext.getRoot().get(datePropertyName), upperBound))));
            }
        }
    ...
    protected <T> OpenmrsFhirCriteriaContext<T> createCriteriaContext(Class<? super T> rootType) {
        ...
        @SuppressWarnings("unchecked") CriteriaQuery<T> cq = (CriteriaQuery<T>) cb.createQuery(rootType);
        @SuppressWarnings("unchecked") Root<T> root = (Root<T>) cq.from(rootType);
    ...
    protected <T> From<?, ?> getRootOrJoin(OpenmrsFhirCriteriaContext<T> criteriaContext, String alias) {
        if (alias.isEmpty()) {
            ...
            return criteriaContext.getJoin(alias).orElseThrow(() -> new IllegalStateException("Tried to reference alias " + alias + " before creating a join with that name"));
        }
    }
