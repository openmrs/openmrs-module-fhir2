...
/**
 * Default handler for the FHIR {@link Patient} resource, mapping onto the OpenMRS {@code Patient}
 * table. Single backing today — claims every incoming Patient. Carries the CRUD wiring lifted from
 * the original {@code FhirPatientServiceImpl}.
 */
@Component
@Order(Ordered.LOWEST_PRECEDENCE)
public class PatientBackedPatientHandler extends BaseFhirResourceHandler<Patient, org.openmrs.Patient> implements FhirResourceHandler<Patient> {
	
	private static final String IMPLICIT_PROFILE = OPENMRS_FHIR_STRUCTURE_DEFINITION_PREFIX + "/openmrs-patient";
	...
	@Override
	public boolean canHandle(@Nonnull Patient resource) {
		return true;
	}
