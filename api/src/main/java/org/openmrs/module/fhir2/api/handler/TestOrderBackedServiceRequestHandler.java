...
/**
 * Default handler for the FHIR {@link ServiceRequest} resource, mapping onto the OpenMRS
 * {@code TestOrder} table. Single backing today — claims every incoming ServiceRequest. Carries the
 * CRUD wiring lifted from the original {@code FhirServiceRequestServiceImpl}.
 */
@Component
@Order(Ordered.LOWEST_PRECEDENCE)
public class TestOrderBackedServiceRequestHandler extends BaseFhirResourceHandler<ServiceRequest, TestOrder> implements FhirResourceHandler<ServiceRequest> {
	
	private static final String IMPLICIT_PROFILE = OPENMRS_FHIR_STRUCTURE_DEFINITION_PREFIX + "/openmrs-servicerequest";
	...
	@Override
	public boolean canHandle(@Nonnull ServiceRequest resource) {
		return true;
	}
